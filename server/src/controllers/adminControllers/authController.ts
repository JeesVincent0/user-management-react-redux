...
const adminAuthController = {
    verifyLogin: async (req: Request, res: Response) => {
        try {
            ...
            if (!user || !user.isAdmin) return res.status(401).json({ status: "failed", message: "Wrong email entered..." });

            const hashPassword = user.hashPassword;

            const checkPassword = await bcrypt.compare(password, hashPassword);

            if (checkPassword) return res.json({ status: "success", user: { name: user.name, email } });
