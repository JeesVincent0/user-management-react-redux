...
const authController = {
    ...
    createUser: async (req, res) => {
        try {
            ...
            const { valid, name, email, password, message } = validator.validator(req.body);
            if (!valid)
                return res.send(401).json({ status: "failed", message });
            if (email && password && name) {
                // checking user existing or not
                const getUser = new userAuthRepositories();
                const user = await getUser.findUserByEmail(email);
                if (user)
                    return res.status(400).json({ status: "failed", error: "Email already taken", user: null });
